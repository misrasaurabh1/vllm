...
class CacheBase:
    ...
    def get_local_cache_path() -> Path:
        return Path(
            os.path.join(cache_dir(), "cache", CacheBase.get_system()["hash"])
        )
...
class PersistentCache(CacheBase):
    ...
    def lookup(
        ...
        log_stats = partial(
            log_global_cache_stats, self.system, op, inputs, precision
        )
        log_vals = partial(
            log_global_cache_vals, self.system, op, inputs, precision
        )
        log_errors = partial(
        ...
        if config.max_autotune or config.max_autotune_gemm:
            local_cache = (
                self.get_local_cache() if config.autotune_local_cache else {}
            )
            ...
            if (
                ...
                try:
                    ...
                    for choice, timing in timings.items():
                        local_cache[op][inputs][precision][
                            choice.hash_key()
                        ] = timing
...
def sha256_hash(data: bytes) -> str:
    ...
    return (
        base64.b32encode(hashlib.sha256(data).digest())[:51]
        .decode("utf-8")
        .lower()
    )
...
def write_atomic(
    ...
    with tmp_path.open(
        write_mode, encoding="utf-8" if encode_utf_8 else None
    ) as f:
        f.write(content)
    try:
...
class FxGraphCachePickler(pickle.Pickler):
        ...
        self.dispatch_table.update(
            {
                ...
                torch.nn.parameter.Parameter: functools.partial(
                    self._reduce_tensor
                ),
    ...
    def _reduce_tensor(
        ...
        Callable[[T], T], tuple[Union[TensorMetadata, TensorMetadataAndValues]]
    ]:
        """
...
def build_code_hash(
    ...
    for lib in sorted(
        pkgutil.iter_modules(roots, prefix), key=lambda x: x.name
    ):
        ...
        if lib.ispkg:
            ...
            build_code_hash(
                spec.submodule_search_locations, f"{spec.name}.", hasher
            )
...
def torch_key() -> bytes:
    ...
    """
    with dynamo_timed(
        "inductor_codecache_torch_key", log_pt2_compile_event=True
    ):
        if not config.is_fbcode():
            ...
            def get_code_hash(root: str) -> bytes:
                ...
                extra_files = [
                    os.path.join(inductor_root, x) for x in extra_files
                ]
        ...
        return (
            parutil.get_file_contents("torch/src_hash.txt")
            .rstrip()
            .encode("ascii")
        )
...
class FxGraphHashDetails:
    ...
    def __init__(
        ...
        if gm is not None:
            for module in gm.modules():
                ...
                for node in itertools.chain(
                    ...
                    kernel_source = user_defined_triton_kernel_transitive_closure_source_code(
                        kernel
                    )
        ...
        no_tensor_inputs = not any(
            isinstance(x, torch.Tensor) for x in example_inputs
        )
...
def compiled_fx_graph_hash(
    ...
    has_user_defined_triton_kernels = (
        len(details.user_defined_triton_source) != 0
    )
...
class FxGraphCache:
    ...
    def _filter_backed_symints(
        ...
        """
        return [
            s for s in inputs if isinstance(s, torch.SymInt) and has_hint(s)
        ]
    ...
    def _save_graph(
        ...
        try:
            ...
            if remote_cache:
                time_taken_ms = int(
                    (disk_compiled_graph._time_taken_ns or 0) // 1e6
                )
        ...
        if compiled_graph is not None:
            ...
            if (time_saved_ns := compiled_graph._time_taken_ns) is not None:
                ...
                if (
                    ...
                    cache_info["ephemeral_timeout_increase"] = (
                        ephemeral_increase
                    )
        else:
            if remote_cache:
...
class CudaKernelParamCache:
    ...
    @classmethod
    def set(
        cls, key: str, params: dict[str, str], cubin: str, bin_type: str
    ) -> None:
        _, path = write(
            cubin,
            bin_type,
    ...
    def compile(
        ...
        *,
        ...
        with lock:
            ...
            log.debug(
                "aot wrapper compilation command: %s", wrapper_compile_cmd
            )
            ...
            if not use_mmap_weights:
                ...
                magic_number = cast(
                    int,
                    torch.randint(0, torch.iinfo(torch.int64).max, (1,)).item(),
                )
            ...
            for custom_obj_idx, (name, constant) in enumerate(
                ...
                log.debug(
                    "saving script object %s as %s", name, custom_obj_name
                )
    ...
    if isinstance(result, (list, tuple)):
        ...
        for i, r in enumerate(result):
            assert isinstance(r, torch.Tensor), (
                op + " returns a list of non-tensors"
            )
...
class HalideCodeCache(CppPythonBindingsCodeCache):
    ...
    def _codegen_glue(cls, meta: HalideMeta, headerfile: object) -> str:
        ...
        glue_template = (
            cls.glue_template_cuda if is_cuda else cls.glue_template_cpp
        )
        ...
        if not os.path.exists(donefile):
            ...
            with FileLock(lockfile, LOCK_TIMEOUT):
                if not os.path.exists(donefile):
                    ...
                    name, output_dir = get_name_and_dir_from_output_file_path(
                        sofile
                    )
...
def cuda_compile_command(
    ...
    if dst_file_ext == "o":
        ...
        raise NotImplementedError(
            f"Unsupported output file suffix {dst_file_ext}!"
        )
...
def get_system() -> dict[str, object]:
    ...
    try:
        ...
        if version_cuda:
            ...
            system["device"]["name"] = getattr(
                device_properties, "gcnArchName", None
            )
...
class DLLWrapper:
    ...
    def __getattr__(self, name: str) -> Callable[..., None]:
        if not self.is_open:
            raise RuntimeError(
                f"Cannot use closed DLL library: {self.lib_path}"
            )
            ...
            with lock:
                ...
                if not os.path.exists(output_path):
                    ...
                    try:
                        ...
                        raise exc.CUDACompileError(
                            cmd_parts, error.output
                        ) from error
                    end_time = time()
    ...
    def compile(
        ...
        if key not in cls.cache:
            ...
            with lock:
                ...
                cls.cache[key] = ROCmCodeCache.CacheEntry(
                    input_path, output_path
                )
