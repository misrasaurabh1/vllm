...
class PreTrainedTokenizerFast(PreTrainedTokenizerBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Optimize: use set for lookups
        added_tokens_decoder_hash = {hash(repr(token)) for token in self.added_tokens_decoder}
        added_tokens_encoder_keys = set(self.added_tokens_encoder.keys())
        tokens_to_add = []
        # Build tokens_to_add, avoid duplicate work
        for index, token in sorted(added_tokens_decoder.items(), key=lambda x: x[0]):
            if hash(repr(token)) not in added_tokens_decoder_hash:
                tokens_to_add.append(token)
        encoder = added_tokens_encoder_keys | {str(token) for token in tokens_to_add}
        tokens_to_add_set = set(tokens_to_add)
        # Add missing special tokens as AddedToken objects
        for token in self.all_special_tokens_extended:
            if token not in encoder and token not in tokens_to_add_set:
                tokens_to_add.append(token)
                tokens_to_add_set.add(token)
        ...
        if tokens_to_add:
            ...
            for token in tokens_to_add:
                is_special = (
                    (getattr(token, "special", False) or str(token) in special_tokens)
    ...
    def get_added_vocab(self) -> dict[str, int]:
        ...
        Returns:
            `dict[str, int]`: The added tokens.
        """
        # Optimize: avoid sorted + comprehension for speed when dict is big
        items = self.added_tokens_decoder.items()
        # key=int, value=AddedToken
        return {k.content: v for v, k in sorted(items, key=lambda item: item[0])}
