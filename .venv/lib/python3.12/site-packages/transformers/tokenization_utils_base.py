...
class PreTrainedTokenizerBase(SpecialTokensMixin, PushToHubMixin):
                ...
                if return_assistant_tokens_mask:
                    ...
                    for i in range(len(input_ids)):
                        ...
                        for (
                            ...
                            start_token = out.char_to_token(
                                i, assistant_start_char
                            )
                            end_token = out.char_to_token(
                                i, assistant_end_char - 1
                            )
    ...
    def _from_pretrained(
        ...
        **kwargs,
        ...
        chat_templates = {}
        chat_template_file = resolved_vocab_files.pop(
            "chat_template_file", None
        )
        extra_chat_templates = [
            key
            for key in resolved_vocab_files
            if key.startswith("chat_template_")
        ]
        if chat_template_file is not None:
            with open(
                chat_template_file, encoding="utf-8"
            ) as chat_template_handle:
            ...
            if "auto_map" in init_kwargs:
                ...
                if isinstance(init_kwargs["auto_map"], (tuple, list)):
                    init_kwargs["auto_map"] = {
                        "AutoTokenizer": init_kwargs["auto_map"]
                    }
        ...
        if "added_tokens_decoder" in init_kwargs:
                ...
                with open(
                    ...
                    for key, value in special_tokens_map.items():
                        ...
                        if isinstance(value, dict):
                            ...
                            additional_special_tokens = (
                                init_kwargs.pop("additional_special_tokens", [])
                                or []
                            )
                            for token in value:
                                if isinstance(token, dict):
            ...
            if added_tokens_file is not None:
                ...
                for key in cls.SPECIAL_TOKENS_ATTRIBUTES & init_kwargs.keys():
                    if init_kwargs[key] is not None:
                        if key == "additional_special_tokens":
                            special_tokens += [
                                str(token) for token in init_kwargs[key]
                            ]
                        else:
            ...
            if tokenizer_file is not None:
                ...
                with open(
                    tokenizer_file, encoding="utf-8"
                ) as tokenizer_file_handle:
        ...
        if (
            added_tokens_decoder != {}
            and max(list(added_tokens_decoder.keys())[-1], 0)
    ...
    def convert_added_tokens(
        ...
        if isinstance(obj, AddedToken) and save:
            ...
            return [
                cls.convert_added_tokens(
                    o, save=save, add_type_field=add_type_field
                )
                for o in obj
            ]
        elif isinstance(obj, dict):
            return {
                k: cls.convert_added_tokens(
                    v, save=save, add_type_field=add_type_field
                )
    ...
    def save_chat_templates(
        ...
        if save_jinja_files and isinstance(self.chat_template, str):
            ...
            tokenizer_config["chat_template"] = [
                {"name": k, "template": v}
                for k, v in self.chat_template.items()
            ]
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        tokenizer_config, saved_raw_chat_template_files = (
            self.save_chat_templates(
                save_directory,
                tokenizer_config,
                filename_prefix,
                save_jinja_files,
    ...
    def pad(
        ...
        if self.__class__.__name__.endswith("Fast"):
            if not self.deprecation_warnings.get(
                ...
                self.deprecation_warnings["Asking-to-pad-a-fast-tokenizer"] = (
                    True
                )
    ...
    def truncate_sequences(
        ...
        if truncation_strategy == TruncationStrategy.ONLY_FIRST or (
            ...
            len_ids = len(ids)
            first_remove = min(
                abs(len_pair_ids - len_ids), num_tokens_to_remove
            )
            ...
            if len_ids > len_pair_ids:
                ...
                pair_ids_to_move = (
                    first_remove + second_remove - (second_remove // 2)
                )
