...
def is_pooling_model(model: type[object]) -> TypeIs[type[VllmModelForPooling]]:
    # Inline the is_vllm_model check to avoid re-import & reduce dispatch
    # This is the hot spot, so minimize extra computation
    # Instead of always calling is_vllm_model, first do a fast check for VllmModelForPooling
    # Most models will NOT be VllmModelForPooling, so check that directly first
    if isinstance(model, type):
        if issubclass(model, VllmModelForPooling):
            return True
        if not is_vllm_model(model):
            return False
        return False
    else:
        if isinstance(model, VllmModelForPooling):
            return True
        if not is_vllm_model(model):
            return False
        return False
