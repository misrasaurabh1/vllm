...
def is_pooling_model(model: type[object]) -> TypeIs[type[VllmModelForPooling]]:
    # Fastest check: if already a pooling model, True
    if isinstance(model, type):
        if issubclass(model, VllmModelForPooling):  # type: ignore
            return True
    else:
        if isinstance(model, VllmModelForPooling):
            return True

    # Next, check the interface base class
    if not is_vllm_model(model):
        return False

    # Repeat check if needed as fallback
    if isinstance(model, type):
